...
async fn account() -> Result<Account, acme::Error> {
    Account::create(
        &NewAccount {
            contact: &[],
...
async fn wait_for_order_rdy(
    ...
    let state = loop {
        ...
        delay *= 2;
        tries += 1;
        info!(?state, tries, "order is not ready, waiting {delay:?}");
        sleep(delay).await;
    };
    ...
    if debug && state.is_err() {
        ...
        info!("Tip: check if the uri's in the above debug traces are reachable");
...
#[tracing::instrument(skip_all)]
pub async fn request(names: Vec<String>, port: u16, debug: bool) -> eyre::Result<Signed> {
    ...
    let state = tokio::select!(
        res = ready => res?,
        e = server => {
            e.wrap_err("Challange server ran into problem")?;
    ...
    if state.status == OrderStatus::Invalid {
        return Err(eyre::eyre!("order is invalid"))
            .suggestion("is the challange server reachable?");
    }
